...
index_vector_labels_per_image_bin = zeros(image_count,kmeans_size);
index_vector_labels_per_image_hist = zeros(image_count,kmeans_size);

%% Kmeans clustring
if run_kmeans
    [lables] = kmeans(Caffe_feats,kmeans_size);
    save('features/caffe_patch/lables','lables');
else
    load('features/caffe_patch/lables');
end
...
for img_idx=1:image_count
    ...
    index_vector_labels_per_image_bin(img_idx,patch_lables)=1;
end
...
for img_idx=1:image_count
    for img_idx2=1:image_count
        %% All similarites (Include non-member of clusters "XNOR")
        similarity_matrix(img_idx,img_idx2)= sum(index_vector_labels_per_image_bin(img_idx,:)== index_vector_labels_per_image_bin(img_idx2,:));
        %% Being in the same clusters ("AND")
...
show_pairs( patch_table(:,1), test_train, Caffe_Index, lables, images_save_path);